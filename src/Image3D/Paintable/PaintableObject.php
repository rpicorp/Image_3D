 ...
 */
class PaintableObject implements Interface_Paintable
{
    ...
    public function subdivideSurfaces($factor = 1)
    {
        for ($i = 0; $i < $factor; ++$i) {
            ...
            foreach ($data['surfaces'] as $surface => $edges) {
                ...
                // Create face point
                $face_points[$surface] = new Point($face_point[0], $face_point[1], $face_point[2]);
            }
            ...
            foreach ($data['edges'] as $edge => $points) {
                ...
                $edge_points[$edge] = new Point(($average_face[0] + $edge_middle[0]) / 2, ($average_face[1] + $edge_middle[1]) / 2, ($average_face[2] + $edge_middle[2]) / 2);
            }
            ...
            foreach ($face_points as $surface => $face_point) {
                ...
                foreach ($points as $point) {
                    ...
                    $this->_addPolygon(new Polygon($old_points[$point],
                                                            $edge_points[$edges[0]],
                                                            $face_point,
                                                            $edge_points[$edges[1]]));
                }
