...
namespace Pear\Image3D\Paintable;

use Pear\Image3D;
use Pear\Image3D\Color;
use Pear\Image3D\Vector;
use Pear\Image3D\Point;
use Pear\Image3D\Interface_Paintable;
use Pear\Image3D\Matrix;
use Pear\Image3D\Line;
use Pear\Image3D\Interface_Enlightenable;
 ...
 */
class Polygon implements Interface_Paintable, Interface_Enlightenable
{
    ...
    protected function _calcNormale()
    {
        ...
        // Backface Culling
        if (($this->_normale->getZ() <= 0) && (@$this->_option[Image3D::IMAGE_3D_OPTION_BF_CULLING])) {
            $this->setInvisible();
        }
    }
    ...
    public function distance(Line $line)
    {
        ...
        foreach ($this->_points as $nr => $point) {
            ...
            $edge = new Vector($nextPoint->getX() - $point->getX(),
                                        $nextPoint->getY() - $point->getY(),
                                        $nextPoint->getZ() - $point->getZ());

            $v = new Vector($cuttingPoint->getX() - $point->getX(),
